...
class TestAgent:
    def __init__(self, ollama_client, github_client, jenkins_client):
        self.ollama = ollama_client
        self.github = github_client
        self.jenkins = jenkins_client
        
    def fetch_tests_from_github(self, owner: str, repo: str, branch: str = "main", test_path: str = "tests/") -> List[Dict]:
        """Pobiera pliki testowe z GitHub repository"""
        try:
            tests = self.github.get_test_files(owner, repo, branch, test_path)
            return tests
        except Exception as e:
            raise Exception(f"Błąd pobierania testów z GitHub: {str(e)}")
    ...
    def apply_fix(self, fix: Dict[str, Any]) -> bool:
        ...
        try:
            # W rzeczywistej implementacji można by tu zaktualizować pliki w repo
